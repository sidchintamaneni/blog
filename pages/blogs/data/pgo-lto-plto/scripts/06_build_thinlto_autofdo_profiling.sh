...
# Step 6: Build ThinLTO + AutoFDO kernel (profiling kernel)

KERNEL_SRC="${KERNEL_SRC:-/home/azure/sid/CBL-Mariner-Linux-Kernel}"
PLTO_CLANG="${PLTO_CLANG:-/home/azure/sid/experiments/os-dev-env/llvm-project/builds/install-O3-ThinLTO-AutoFDO-propeller/bin}"
...
scripts/config --set-str LOCALVERSION "-thinlto-autofdo-build"

# Enable ThinLTO + AutoFDO
...
BUILD_LOG="$(mktemp)"
{ time make -j "$(nproc)" ARCH=x86_64 LLVM="${PLTO_CLANG}/"; } 2> "${BUILD_LOG}"
...
# Keep a copy of vmlinux for llvm-profgen (needs the matching binary)
mkdir -p "${KERNEL_SRC}/builds/clang-thinlto-autofdo"
cp vmlinux "${KERNEL_SRC}/builds/clang-thinlto-autofdo/"
...
echo
echo "========== RESULTS (ThinLTO + AutoFDO, profiling kernel) =========="
...
echo "=================================================================="
echo "Next: reboot into ${KVER}, then run Step 7 to collect the perf profile."
