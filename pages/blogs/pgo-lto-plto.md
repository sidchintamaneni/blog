...
representation (IR), and backend. The frontend parses the source code (things like
...
combined IR enabling cross-module inlining, dead code elimination, etc before
the backend generates the final object code and links it. As one can imagine
this is a memory intensive process. Fun fact I've done this in the past to
generate a callgraph for a subset of kernel functions[8] using `make
...
runs on a system? That's where PGO comes in.
...
read more about JVM JIT. Static binary optimizers take a different approach,
they rewrite the static binary, ahead of execution. No runtime overhead,
simpler deployment. BOLT[15] is a static binary optimizer. 

BOLT reads the final linked binary, disassembles it, and reconstructs the
control flow graph for each function. Using sample-based profiles, it runs an
...
pass, so both memory and time overhead grows quickly with binary size. 

Propeller[18] takes a different approach - instead of disassembling and
rewriting the binary, it relinks it (what does it even mean?). The compiler
...
This ordering is fed back as layout hints then the compiler uses them to
reorder blocks within each function, and the linker performs the global
function layout during the final link step.
...
exceed the 128GB of available memory on the system (BUG?!). So for all the experiments I've
...
Rather than optimizing all code equally, they focus optimization effort on the
regions that dominate execution time. But how does the compiler know during
build time about the critical areas in the programs? that is where profiling/
sampling comes in.
...
kept filling up my disk (1 TB) so I gave up.
...
with iFDO due to function splitting (hot-cold splitting) and less aggressive
inilining. For AutoFDO, probably because of poor profiling quality there is
aggressive inlining of functions which resulted in decrease in function count
and increase in binary size.
...
I ignored iFDO because of the nearly comparable benchmarking results when I ran
BOLT + AutoFDO. Since AutoFDO is much preferred approach, I've ignored it.
