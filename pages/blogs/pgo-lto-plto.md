...
intermediate representation and backend. Front parses the source code (things
...
all the IR files (bitcode files) and generates a one monolithic file and then
...
the past to generate a callgraph for subset of kernel functions[8]. To avoid
this ppl have come up with ThinLTO. 
...
The idea is to collect profiling data from a program during runtime and feed it
